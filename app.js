(() => {
  ...
  function renderBatches() {
    ...
    filtered.forEach(batch => {
      ...
      fragment.querySelector('.tencent-copy-btn').addEventListener('click', event => {
        event.stopPropagation();
        copyBatchToTencentDocs(batch.id, event.currentTarget);
      });
  ...
  async function exportBatchExcel(batchId, button = null) {
    ...
    try {
      const workbookBlob = await buildBatchXlsxWithCellImages(batch);
      ...
      showToast('Excel 已导出：图片位于单元格中，点击图片单元格可查看原图');
    } catch (error) {
      console.error(error);
      ...
      if (button) {
        ...
        button.textContent = originalText || '导出 Excel（单元格图片）';
      }
    }
  }
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${relationships}</Relationships>`;
  ...
  async function copyBatchToTencentDocs(batchId, button = null) {
    ...
    try {
      ...
      if (button) {
        button.disabled = false;
        button.textContent = originalText || '复制到腾讯文档';
      }
    }
  }
